...
impl GraphicalUserInterface {
                ...
                egui::CentralPanel::default().show(ctx, |ui| {
                    ...
                    if ui.button("login").clicked() {
                        ...
                        let (secret_key, public_key, wrapped_user_key, user_key_nonce) =
                            general_login(
                                &mut self.string_values.master_login.password.take(),
                                &self.data_base_header.expect("unreachable").master_pw_salt,
                            );
    ...
    fn graphical_user_interface_add_user_password(
        &mut self,
        context: &Context,
        ...
        CommandBuilder::new("add user password", "add user password")
            ...
            .user_key(self.wrapped_user_key.as_ref().expect("unreachable"))
            .user_key_nonce(self.user_key_nonce.as_ref().expect("unreachable"))
            .execute(
                |inputs, data_base, wrapped_user_key, _, user_key_nonce, _| {
                    ...
                    add_user_pw(
                        ...
                        user_identifier,
                        user_password,
                        wrapped_user_key.expect("unreachable"),
                        user_key_nonce.expect("unreachable"),
                    )?;
                    Ok(())
                },
            )
    ...
    fn graphical_user_interface_get_user_password(&mut self, context: &Context, button: egui::Response) {
        CommandBuilder::new("get user password", "get user password")
            .input("site name", &mut self.string_values.command_value.get_user_password.site_names)
            ...
            .execute(|inputs, data_base, wrapped_user_key, _, _, _| {
                let site_names = SiteName::new(&inputs[0].value)?;
                let user_identifier = UserID::new(&inputs[1].value)?;
                let password = get_user_pw(data_base.expect("unreachable"), &site_names, &user_identifier, wrapped_user_key.expect("unreachable"))?;
                Ok(password)
            })
            .on_success(|password| {
    ...
    */
    ...
    fn graphical_user_interface_change_master_password(
        ...
        CommandBuilder::new("change master password", "change master password")
            ...
            .database(&mut self.data_base)
            .user_key_mut(self.wrapped_user_key.as_mut().expect("unreachable"))
            .user_key_nonce_mut(self.user_key_nonce.as_mut().expect("unreachable"))
            .execute(
                |inputs, data_base, _, wrapped_user_key_mut, _, user_key_nonce_mut| {
                    ...
                    let (public_key, salt) = change_master_pw(
                        data_base,
                        inputs[0].value.take(),
                        wrapped_user_key_mut.expect("unreachable"),
                        user_key_nonce_mut.expect("unreachable"),
                    )?;
                    ...
                    save_db(
                        self.data_base_header.as_mut().expect("unreachable"),
...
impl eframe::App for GraphicalUserInterface {
                    ...
                    egui::Grid::new("user_data").show(ui, |ui| {
                        ...
                        if button.clicked() {
                            self.window_open_list
                                .user_state
                                .update_site_name(site_name.clone(), true);
                        }
                    });
                }
                {
                    ...
                    for (site_name, is_open) in site_names {
                        if let Some(passwords) = self.data_base.get_mut(site_name)
                            && *is_open
                        {
                            ctx.show_viewport_immediate(
                                ...
                                egui::ViewportBuilder::default()
                                    .with_title(format!("{} user data", site_name.as_str())),
                                |ctx, _| {
                                    if ctx.input(|input_state| {
                                        input_state.viewport().close_requested()
                                    }) {
                                    ...
                                    egui::CentralPanel::default().show(ctx, |ui| {
                                        ui.label(site_name.as_str());
                                        for (user_identifier, encrypted_password) in
                                            passwords.iter_mut()
                                        {
                                            egui::Grid::new(user_identifier)
                                            .show(
                                                ui,
                                                |ui| {
                                                    ui.horizontal(|ui| {
                                                        ...
                                                        if button.is_pointer_button_down_on() {
                                                            let mut user_password =
                                                                match decrypt_user_pw(
                                                                    site_name,
                                                                    user_identifier,
                                                                    encrypted_password,
                                                                    self.wrapped_user_key
                                                                        .as_ref()
                                                                        .expect("unreachable"),
                                                                    self.user_key_nonce
                                                                        .as_ref()
    ...
    fn on_exit(&mut self, _gl: Option<&eframe::glow::Context>) {
        let encrypted_data_base = encrypt_db(
            &self.data_base,
            self.public_key.as_ref().expect("unreachable"),
        );
        save_db(
            &mut self.data_base_header.expect("unreachable"),
            encrypted_data_base,
        )
        .expect("unreachable");
    }
