...
use crate::sodium_bindings::{
    crypto_aead_aes256gcm_ABYTES, crypto_aead_aes256gcm_decrypt, crypto_aead_aes256gcm_encrypt,
    randombytes_buf,
};
use alloc::vec::Vec;
...
pub fn aes256gcm_encrypt(key: &AesKey, nonce: &AesNonce, plaintext: &[u8]) -> Vec<u8> {
    ...
    unsafe {
        ciphertext.set_len(ciphertext_len as usize);
    }
    unsafe {
        crypto_aead_aes256gcm_encrypt(
            ciphertext.as_mut_ptr(),
            addr_of_mut!(actual_ciphertext_len),
            plaintext.as_ptr(),
    ...
    assert_eq!(
        actual_ciphertext_len, ciphertext_len,
        "AES-GCM ciphertext length mismatch of {{ plaintext length + verifier tag langth (16) }}"
    );
    ciphertext
}
...
pub fn aes256gcm_decrypt(
    ...
    let plaintext_len =
        ciphertext.len() as c_ulonglong - crypto_aead_aes256gcm_ABYTES as c_ulonglong;
    let mut plaintext = SodiumBox::<c_uchar>::new_with_size(plaintext_len as usize);
    let rc = unsafe {
        crypto_aead_aes256gcm_decrypt(
            plaintext.as_mut_ptr(),
            addr_of_mut!(actual_plaintext_len),
            null_mut(),
            ciphertext.as_ptr(),
    ...
    assert_eq!(
        actual_plaintext_len, plaintext_len,
        "AES-GCM ciphertext length mismatch of {{ plaintext length - verifier tag langth (16) }}"
    );
    Ok(plaintext)
}
