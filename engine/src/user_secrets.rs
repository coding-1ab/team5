...
use libsodium_sys::rust_wrappings::aes256gcm::{
    AES_KEY_SIZE, AES_NONCE_SIZE, AES_OUT_AUTH_TAG_SIZE, AesKey, AesNonce, aes256gcm_decrypt,
    aes256gcm_decrypt_from_ptr, aes256gcm_encrypt, aes256gcm_encrypt_from_ptr_to_sodium_box,
};
...
#[derive(Archive, Deserialize, Serialize)]
pub struct EncryptedUserID(#[rkyv(with = SecretBoxRef)] SecretBox<[u8]>);
...
#[derive(Archive, Deserialize, Serialize)]
pub struct EncryptedUserPW(#[rkyv(with = SecretBoxRef)] SecretBox<[u8]>);
...
impl SessionKey {
    ...
    pub fn gen_rand() -> Self {
        Self {
            inner: SodiumBox::new_with_size(Self::SIZE),
        }
    }
    fn from_sodium_box(sodium_box: SodiumBox<u8>) -> Self {
        Self { inner: sodium_box }
...
pub fn get_session_key_wrapper() -> SessionKeyWrapper {
    ...
    hasher.update(&[
        248, 106, 27, 141, 130, 70, 18, 189, 65, 15, 132, 220, 144, 144, 143, 196, 57, 128, 134,
        145, 197, 235, 192, 209, 150, 152, 201, 113, 12, 189, 100, 93, 92, 69, 244, 146, 157, 57,
        131, 56, 143, 160, 17, 233, 114, 23, 32, 13, 68, 9, 116, 95, 26, 104, 73, 81, 7, 7, 103,
        206, 63, 251, 161, 223, 226, 125, 184, 225, 6, 164, 65, 13,
    ]);
    ...
    let mut start_time = sys.process(s_pid).map(|p| p.start_time()).unwrap();
    combined_pids.zeroize();
...
fn get_user_pw_nonce(site: &SiteName, id: &UserID) -> UserPWNonce {
    ...
    let halo = [
        203u8, 118, 6, 1, 225, 226, 197, 127, 221, 214, 24, 5, 239, 38, 75, 82, 65, 111, 91, 110,
        158, 25, 48, 178, 116, 137, 136, 49, 57, 192, 56, 52,
    ];
...
pub fn wrap_session_key(session_key: SessionKey) -> (WrappedSessionKey, SessionKeyNonce) {
    let wrapper = get_session_key_wrapper().into();
    let nonce = SessionKeyNonce::gen_rand();

    let ciphertext = aes256gcm_encrypt_from_ptr_to_sodium_box(
        &wrapper,
        nonce.as_ref(),
        session_key.as_ptr(),
        SESSION_KEY_SIZE,
...
pub fn unwrap_session_key(
    ...
    let wrapper = get_session_key_wrapper().into();
    let plaintext = aes256gcm_decrypt_from_ptr(
        &wrapper,
        nonce.as_ref(),
        wrapped_key.as_ptr(),
...
pub fn encrypt_user_pw(
    ...
    let ciphertext = aes256gcm_encrypt(&session_key, &user_pw_nonce, user_pw.as_str().as_bytes());
    drop(session_key);
    drop(user_pw_nonce);
...
#[inline(always)]
pub fn decrypt_user_pw(
    site: &SiteName,
    id: &UserID,
    encrypted_pw: &EncryptedUserPW,
    wrapped_key: &WrappedSessionKey,
    user_key_nonce: &SessionKeyNonce,
) -> Result<UserPW, DBIOError> {
    let user_key: AesKey = unwrap_session_key(wrapped_key, user_key_nonce)?.into();
    let user_pw_nonce: AesNonce = get_user_pw_nonce(site, id).into();
    let plaintext = aes256gcm_decrypt(&user_key, &user_pw_nonce, encrypted_pw.as_bytes())
        .map_err(|_| DBIOError::InvalidSession)?
        .into();
    drop(user_key);
    drop(user_pw_nonce);

    let user_pw = UserPW::from_unchecked(String::from_utf8(plaintext).unwrap());
